'''wrangle contains helper functions to assist in
...
rename_dict = {
    ...
    'calculatedbathnbr': 'calc_bath_and_bed',
    'finishedfloor1squarefeet': 'finished_floor1_sqft',
    'calculatedfinishedsquarefeet': 'calc_sqft',
    'finishedsquarefeet12': 'finished_sqft12',
    'finishedsquarefeet13': 'finished_sqft13',
    'finishedsquarefeet15': 'finished_sqft15',
    'finishedsquarefeet50': 'finished_sqft50',
    'finishedsquarefeet6': 'finished_sqft6',
    ...
    'lotsizesquarefeet': 'lot_sqft', 'poolcnt': 'pool_count',
    'poolsizesum': 'sum_pool_size',
    ...
    'regionidneighborhood': 'region_id_neighbor',
    ...
    'heatingorsystemdesc': 'heating_system_desc',
...
def get_zillow_from_sql() -> pd.DataFrame:
    '''
    reads MySQL data from `zillow` database and returns `pandas.DataFrame`
    with raw data from query
    # Parameters
    None
    # Returns
...
def df_from_csv(path: str) -> Union[pd.DataFrame, None]:
    '''
    returns zillow DataFrame from .csv if it exists at `path`,
...
def wrangle_zillow(from_sql: bool = False, from_csv: bool = False,
    ...
    '''
    wrangles Zillow data from either a MySQL query or a `.csv` file,
    prepares the data, and returns a `pandas.DataFrame` object
    ...
    unprepared states
    ## Parameters
    refresh: if `True`, ignores any `.csv` files and pulls new data
    ...
    return prep_zillow(ret_df, prop_row=prop_row, prop_col=prop_col,
                       outlier_k=outlier_k, bound=bound)
...
def prep_zillow(df: pd.DataFrame, prop_row: float, prop_col: float,
    ...
    df.loc[df.calc_bath_and_bed.isna(
    ), 'calc_bath_and_bed'] = na_bath.bed_count + na_bath.bath_count
...
def handle_null_cols(df: pd.DataFrame, pct_col: float) -> pd.DataFrame:
    ...
    pct_col: Float `0 <= pct_col <=1`
    indicating what percentage of non-null values
...
def handle_missing_values(
        df: pd.DataFrame, pct_row: float, pct_col: float) -> pd.DataFrame:
    '''
...
def tvt_split(dframe: pd.DataFrame, stratify: Union[str, None] = None,
    ...
    '''
    Splits data set into train, validate, and test data sets
    ## Parameters
    dframe: `DataFrame` containing input data
    stratify: `str` specifying `stratify` variable (if applicable)
    tv_split: `float` specifying split percentage for train_validate
    and test (initial split)
    validate_split: `float` specifying split percentage for train
    and validate split
    sample: percentage sample of data to return for larger
    data sets (if applicable)
    ## Returns
    `Tuple` containing train, validate, and test data sets
    '''
    if stratify is not None:
        stratify = dframe[stratify]
    train_validate, test = train_test_split(
        dframe, test_size=tv_split,
...
def scale_data(train: pd.DataFrame, validate: pd.DataFrame, test: pd.DataFrame,
    ...
    validate: `DataFrame` of validate data
