...
def scale_and_cluster(df: pd.DataFrame, features: List[str],
                      ...
                      k: Union[int, None] = None) -> Tuple[pd.DataFrame,
                                                           ...
                                                           KMeans]:
    '''
    scales and clusters the data in `df`
    ## Parameters
    ...
    ## Returns
    a `DataFrame` scaled and clustered according to `scaler` and the KMeans
...
def generate_regressor(df: pd.DataFrame, features: List[str],
    ...
    '''
    Creates a regression model for each cluster in `cluster_name`
    ## Parameters
        df: `DataFrame` containing modeling data
    features: List of strings of the columns containing features
    to model on
    target: string of the column in `df` containing the target
    variable
    regressor: an initialized regressor object(e.g. TweedieRegressor,
    LassoLars,LinearRegression) to be used on models
    ## Returns
    a dictionary mapping the integers indicating clusters to their 
...
def apply_to_clusters(df: pd.DataFrame, features: str, target: str,
    ...
    '''
    applys the individual cluster models generated by `generate_regressor`
    ...
    target: string of the column in `df` containing the target
    variable
    cluster_name: name of the column in `df` containing cluster
    indicators
    regressors: dictionary mapping integers of the individual
    clusters to their respective regression model
    ## Returns
    a DataFrame containing the true and predicted Y values
    '''
